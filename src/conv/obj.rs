...
pub fn jsonseq_file_to_obj(path: impl AsRef<Path>, output_path: impl AsRef<Path>) -> IoResult<()> {
    ...
    for (i, line) in br.lines().enumerate() {
        let l = line?;
        if i == 0 {
            cjj = CityJSON::from_str(&l).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e))?;
        } else {
            let mut cjf = CityJSONFeature::from_str(&l).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e))?;
...
/// 
/// A result indicating success or an I/O error.
fn convert_geometry_to_obj<W: Write>(boundaries: &crate::Boundaries, writer: &mut W) -> IoResult<()> {
    match boundaries {
